...
export async function collectSettings(): Promise<SettingRow[]> {
  ...
  rows.push({
    ...
    key: "auth.provider",
    label: "Auth provider",
    value: "password (hesla rolí z env)",
  });
  rows.push({
    group: core,
    key: "auth.sessionTtl",
  ...
  rows.push({
    group: core,
    key: "modules",
    label: "Zapnuté core moduly",
    value: enabledList(coreModules),
  });
  rows.push({
    group: core,
    key: "projects.root",
    label: "Data projektů",
    value: "GitHub content storage (stateless)",
  });
  rows.push({
    group: core,
    key: "projects.active",
    label: "Aktivní projekty",
    value: adminConfig.activeProjectIds.length
      ? adminConfig.activeProjectIds.join(", ")
      : "(všechny registrované)",
  });
  rows.push({
    group: core,
    key: "deploy.hooks",
  ...
  for (const project of listProjects()) {
    ...
    rows.push({
      ...
      key: "features",
      label: "Feature flags",
      value: enabledList(project.features),
    });
    rows.push({
      group,
      key: "media.provider",
